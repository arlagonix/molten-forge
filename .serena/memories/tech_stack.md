...
- **IPC bridge** via `contextBridge.exposeInMainWorld` with 6 namespaces:
  - `codeForgeAI` — AI chat streaming, attachments
  - `chatForgeStorage` — KV store + chat CRUD
  - `chatForgeWorkspace` — folder picker
  - `chatForgeTools` — tool execution (file edit, bash, etc.)
  - `chatForgeFind` — find-in-page
  - `chatForgeMcp` — MCP tool refresh/execute
